                ...
                <div v-for="mensagem in mensagens" :key="mensagem.id" :class="['p-2 rounded-md max-w-[80%]', mensagem.usuario_id === usuario_id ? 'bg-blue-100 self-end text-right' : 'bg-gray-100 self-start text-left']">
                    ...
                    <div class="text-sm text-gray-600">
                        <template v-if="mensagem.mensagem === 'Imagem'">
                            <img :src="mensagem.imagem ? `http://localhost:8000/storage/${mensagem.imagem}` : mensagem.urlImagem" alt="Imagem enviada" class="max-w-xs max-h-48 rounded" />
                        </template>
                        <template v-else>
                            {{ mensagem.mensagem }}
                        </template>
                    ...
                    <Column header="Ações" style="width: 100px" headerClass="font-medium text-gray-600 text-xs uppercase">
                        <template #body="{ data }">
                            <div class="flex items-center gap-1">
                                <Button @click.prevent="visualizarChamado(data.id)" style="color: #004285" icon="pi pi-comments" class="p-button-rounded p-button-text p-button-sm text-blue-500 hover:bg-blue-50" v-tooltip.top="'Visualizar'" />
                            </div>
                        </template>
                    </Column>
                </DataTable>
